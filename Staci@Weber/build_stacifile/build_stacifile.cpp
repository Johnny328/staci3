...
/// compile: clang++ -o build_stacifile build_stacifile.cpp
/// TO SET: tol, filter, filter2

int main(int argc, char* argv[]){

  // INPUT
  double tol = 0.0; // minimal distance between end of pipelines
  double filter = 20.; // deleting pipelines shorter that "filter" and having only one connected end i.e. endpipelines
  double filter2 = 50.; // deleting pipelines shorter than "filter2" and having rank of 2 at both ends
  double gv_dist_min = 100.;
  ...
  //cout << "\nNODES\n----------" << endl;
  //for(int i=0; i<node.size(); i++)
  //  printf("%5s|%10.2f|%10.2f|%3i\n", node[i].c_str(),x[i],y[i],rank[i]);

  //cout << "\nEDGES\n----------\n";
  //for(int i=0; i<id.size(); i++)
  //  printf("%10s|%5s|%5s\n", id[i].c_str(), node_from[i].c_str(), node_to[i].c_str());
