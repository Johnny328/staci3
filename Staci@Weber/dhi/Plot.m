...
caseFolder = '../../Networks/';
addpath('../../Plot');
% projectFolder = '../Staci@Weber/shutdownEffect/';
...
colorMapName = 'viridis'; %see ColorMaps folder
colorBarText = 'Pressure [mwc]';
...
for i=1:pipeCounter
    ...
    plot([node(pipe(i).nodeFromIdx).coordX,node(pipe(i).nodeToIdx).coordX],[node(pipe(i).nodeFromIdx).coordY,node(pipe(i).nodeToIdx).coordY],'k','linewidth',lineWidth,'color',[r,g,b]);
end
for i=1:pumpCounter
    if(colorElement == "All")
    ...
    PlotPump([node(pump(i).nodeFromIdx).coordX,node(pump(i).nodeToIdx).coordX],[node(pump(i).nodeFromIdx).coordY,node(pump(i).nodeToIdx).coordY],pumpRadius,1,[r,g,b]);
end
for i=1:presCounter
    if(colorElement == "All")
    ...
    PlotPres(pres(i).coordX,pres(i).coordY,presRadius,1,90,[r,g,b]);
end
for i=1:poolCounter
    if(colorElement == "All")
    ...
    PlotPool(pool(i).coordX,pool(i).coordY,tankSize,1,0,[r,g,b]);
end
for i=1:valveCounter
    PlotValve([node(valve(i).nodeFromIdx).coordX,node(valve(i).nodeToIdx).coordX],[node(valve(i).nodeFromIdx).coordY,node(valve(i).nodeToIdx).coordY],valveSize,1);
end
for i=1:nodeCounter
    ...
    plot(node(i).coordX,node(i).coordY,'ko','linewidth',lineWidth,'markersize',nodeMarkerSize,'markerfacecolor',[r,g,b],'color',[r,g,b]);
end
